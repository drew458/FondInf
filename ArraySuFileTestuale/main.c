 ...
 * reali su un file testuale e la lettura di una sequenza di numeri reali da file. 
...
void scriviSuFile() {
	...
	FILE* fp = fopen("numeriRealiTestuale.txt", "w");
	if(fp==NULL)
...
int dimensioneFile() {
		...
		FILE* fp = fopen("numeriRealiTestuale.txt", "r");
		if(fp!=NULL) {
			while(fgetc(fp)!=EOF){
				fscanf(fp, "%f", &numero);
				n++;
